...
export function exportRankingExcel(ranking: Ranking) {
  const executiveRows = ranking.entries.map((entry) => `<tr><td>${entry.position}º</td><td>${escapeHtml(entry.name)}</td><td>${escapeHtml(entry.team)}</td><td>${entry.plates}</td><td>${entry.revenue.toFixed(2)}</td><td>${entry.averageTicket.toFixed(2)}</td><td>${entry.movement}</td></tr>`).join("");
  const teamRows = (ranking.teamEntries ?? []).map((entry) => `<tr><td>${entry.position}º</td><td>${escapeHtml(entry.team)}</td><td>${entry.cooperativeCode}</td><td>${entry.members}</td><td>${entry.plates}</td><td>${entry.revenue.toFixed(2)}</td><td>${entry.averageTicket.toFixed(2)}</td><td>${entry.movement}</td></tr>`).join("");
