...
type ArtworkTarget = "executives" | "teams";
...
async function drawArtwork(canvas: HTMLCanvasElement, ranking: Ranking, executives: Executive[], format: ArtFormat, target: ArtworkTarget) {
  const width = 1080;
  ...
  context.fillStyle = "#ffffff";
  context.font = `900 ${format === "story" ? (target === "teams" ? 76 : 94) : (target === "teams" ? 63 : 76)}px Arial`;
  ...
  const top = target === "executives"
    ? ranking.entries.slice(0, 3).map((entry) => {
      ...
      return { id: entry.normalizedName, name: artworkName(entry, executive), secondary: entry.team, plates: entry.plates, revenue: entry.revenue, averageTicket: entry.averageTicket, executive };
    })
    : (ranking.teamEntries ?? []).slice(0, 3).map((entry) => ({
      id: entry.teamId, name: entry.team, secondary: `${entry.members} ${entry.members === 1 ? "PARTICIPANTE" : "PARTICIPANTES"}`, plates: entry.plates, revenue: entry.revenue, averageTicket: entry.averageTicket, executive: undefined,
    }));
  ...
  const loaded = await Promise.all(top.map(async (entry) => {
    const executive = entry.executive;
  ...
  top.forEach((entry, index) => {
    ...
    context.fillText(ellipsize(context, entry.secondary.toUpperCase(), position.maxWidth), position.x, nameY + (compact ? 29 : 37));
    context.fillStyle = "#ffc400";
...
export function ArtworkGenerator() {
  ...
  const [target, setTarget] = useState<ArtworkTarget>("executives");
  ...
  const missingPhotos = useMemo(() => target === "executives" ? ranking?.entries.slice(0, 3).filter((entry) => !entry.executiveId || !executiveById.get(entry.executiveId)?.fotoDataUrl) ?? [] : [], [ranking, executiveById, target]);

  useEffect(() => {
    if (!ranking || !canvasRef.current) return;
    setDrawing(true);
    drawArtwork(canvasRef.current, ranking, executives, format, target).finally(() => setDrawing(false));
  }, [ranking, executives, format, target]);

  function download() {
    if (!canvasRef.current || !ranking) return;
    const anchor = document.createElement("a");
    anchor.download = `ranking-br-${target === "teams" ? "equipes" : "executivos"}-${ranking.periodEnd}-${format}.png`;
  ...
  return <div className="artwork-layout">
    <aside className="panel artwork-controls"><span className="section-kicker">Gerador de imagem</span><h2>Arte do Top 3</h2><p>Gere o pódio de executivos ou equipes com os indicadores congelados no fechamento.</p>
      ...
      <label className="form-label"><span>Ranking da arte</span><select className="text-input" value={target} onChange={(event) => setTarget(event.target.value as ArtworkTarget)}><option value="executives">Top 3 executivos</option><option value="teams">Top 3 equipes</option></select></label>
